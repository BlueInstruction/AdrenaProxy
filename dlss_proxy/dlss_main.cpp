...
__declspec(dllexport) int DLSS_GetOptimalSettings(
    ...
    *outSharpness = cfg.sharpness;
    ...
    // Init SGSR pipeline if needed
    if (g_device && !g_sgsr.IsInitialized()) {
...
__declspec(dllexport) int DLSS_Evaluate(
    ...
    // Try multiple key variants for color input
    ...
    // Run SGSR upscaling
    if (output && color && g_sgsr.IsInitialized()) {
