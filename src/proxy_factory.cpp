...
{
    AD_LOG_I("ProxyFactory created (wrapping %p)", real);
}
...
HRESULT ProxyFactory::CreateSwapChain(IUnknown* dev, DXGI_SWAP_CHAIN_DESC* desc, IDXGISwapChain** sc)
{
    ...
    HRESULT hr = m_real->CreateSwapChainForHwnd(dev, desc->OutputWindow, &desc1, nullptr, output, &sc1);
    if (SUCCEEDED(hr) && sc1 && sc) {
        *sc = sc1; // sc1 is already a wrapped swapchain from CreateSwapChainForHwnd
        return S_OK;
    }
    return hr;
}

HRESULT ProxyFactory::CreateSoftwareAdapter(UINT i, IDXGIAdapter** a) { return m_real->CreateSoftwareAdapter(i,a); }
    ...
    const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* fsDesc, IDXGIOutput* output, IDXGISwapChain1** sc)
{
    ...
    if (FAILED(hr)) {
        AD_LOG_E("CreateSwapChainForHwnd failed: 0x%08X", hr);
        return hr;
    }
    return WrapSwapChain(dev, realSC, sc);
}
...
HRESULT ProxyFactory::UnregisterStereoStatus(DWORD c) { return m_real->UnregisterStereoStatus(c); }
...
HRESULT ProxyFactory::UnregisterOcclusionStatus(DWORD c) { return m_real->UnregisterOcclusionStatus(c); }
...
// IDXGIFactory6
HRESULT ProxyFactory::EnumAdapterByGpuPreference(UINT i, DXGI_GPU_PREFERENCE p, REFIID r, void** v) {
    return m_real->EnumAdapterByGpuPreference(i, p, r, v);
}
...
HRESULT ProxyFactory::WrapSwapChain(IUnknown* pDevice, IDXGISwapChain1* realSC, IDXGISwapChain1** ppSC)
{
    ...
    auto* proxy = new ProxySwapChain(realSC, pDevice);
    *ppSC = proxy;
    return S_OK;
}
